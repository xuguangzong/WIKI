...
from DjangoWiki.utils import send_email, get_md5, save_user_avatar
...
from DjangoWiki.wiki_signals import oauth_user_login_signal
...
def authorize(request):
    ...
    if user:
        if not user.nikename or not user.nikename.strip():
            import datetime
            user.nikename = "djangowiki" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
        try:
                ...
                if not author:
                    ...
                    if result[1]:
                        try:
                            ...
                            author.username = "djangowiki" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
    ...
    with transaction.atomic():
        if oauthuser.author:
            ...
            if result[1]:
                ...
                ) else "djangowiki" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
                author.save()
