...
class WikiMarkDownRenderer(mistune.Renderer):
    '''
    markdown渲染
    '''
...
class CommonMarkdown():
    ...
    def get_markdown(value):
        renderer = WikiMarkDownRenderer(inlinestyles=False)
...
def send_email(emailto, title, content):
    from DjangoWiki.wiki_signals import send_email_signal
...
def get_wiki_setting():
    value = cache.get('get_wiki_setting')
    if value:
        return value
    else:
        from wiki.models import WikiSettings
        if not WikiSettings.objects.count():
            ...
            setting.save()
        value = WikiSettings.objects.first()
        logger.info('set cache get_wiki_setting')
        cache.set('get_wiki_setting', value)
        return value
...
def delete_sidebar_cache(username):
    ...
    from wiki.models import LinkShowType
    keys = (
