...
from servermanager.Api.wikiapi import WikiApi
from servermanager.Api.commonapi import TuLing
import os
import json
from DjangoWiki.utils import get_md5
from django.conf import settings
import jsonpickle
from servermanager.models import commands

robot = WeRoBot(token=os.environ.get('DJANGO_WEROBOT_TOKEN'), enable_session=True)
...
def convert_to_articlereply(articles, message):
    ...
    from wiki.templatetags.wiki_tags import custom_markdown, truncatechars_content
...
def search(message, session):
    ...
    result = wikiapi.search_articles(searchstr)
    if result:
...
def category(message, session):
    categorys = wikiapi.get_category_lists()
...
def recents(message, session):
    articles = wikiapi.get_recent_articles()
    if articles:
        reply = convert_to_articlereply(articles, message)
        return reply
    else:
